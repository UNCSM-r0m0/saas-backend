  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit
{
  ...
  private readonly MAX_PER_USER = Number(
    process.env.CHAT_MAX_STREAMS_PER_USER ?? 1,
  );
  private readonly QUEUE_TIMEOUT_MS = Number(
    process.env.CHAT_QUEUE_TIMEOUT_MS ?? 8000,
  );
  private readonly QUEUE_MAX_WAITERS = Number(
    process.env.CHAT_QUEUE_MAX_WAITERS ?? 50,
  );
    ...
    try {
      ...
      if (token) {
        try {
          ...
          this.logger.warn(
            `❌ Token inválido para cliente: ${client.id}`,
            error.message,
          );
  ...
  private tryDrainQueue() {
    for (let i = 0; i < this.waitQueue.length; i++) {
      ...
      if (this.canStart(t.userKey)) {
        ...
        this.userActive.set(
          t.userKey,
          (this.userActive.get(t.userKey) || 0) + 1,
        );
  ...
  private releaseSlot(userKey: string) {
    ...
    this.userActive.set(
      userKey,
      Math.max(0, (this.userActive.get(userKey) || 1) - 1),
    );
    this.tryDrainQueue();
  }
  ...
  private emitChat(
    ...
    if (broadcast) {
      ...
      this.logger.log(
        `📡 [EMIT] ${event} → sala ${chatId} (${clientCount} clientes)`,
      );
      ...
      if (isCritical && !isClientInRoom) {
        this.logger.log(
          `📡 [EMIT] ${event} → cliente directo ${client.id} (backup crítico - no en sala)`,
        );
  ...
  private async processMessageInBackground(
    ...
    try {
      ...
      const history = userId
        ? await this.chatService.getChatHistory(chatId)
        : [];
      const messages = [
        ...history,
        ...
        while (i < text.length) {
          if (text.startsWith('```', i)) {
            if (!openCodeBlock) {
              ...
              const fenceLine = text
                .slice(i + 3, nl === -1 ? text.length : nl)
                .trim();
      ...
      const flush = (force = false) => {
        ...
        this.logger.debug(
          `📤 [CHUNK ${seq}] Enviando a sala ${chatId}: ${payload.content.substring(0, 50)}...`,
        );
        buffer = '';
      };
      ...
      try {
        ...
        if (model === 'gemini') {
          ...
          stream = await this.openaiService.generateStreamingResponse(
            openaiMessages,
            { model },
          );
        ...
        if (aborted) {
          this.logger.warn(
            `Stream aborted (client disconnected) ${chatId} after ${chunkCount} chunks`,
          );
          return;
        }

        this.logger.log(
          `Stream completed for ${chatId} (${chunkCount} chunks batched)`,
        );
        ...
        if (buffer.length > 0 || fullContent.length > 0) {
          flush(true);
          this.logger.log(
            `Enviado contenido parcial (${fullContent.length} chars) antes del error`,
          );
        }
        ...
        const errorCause =
          streamError?.cause?.message || streamError?.cause?.code || '';
        const nestedCause =
          streamError?.cause?.cause?.code ||
          streamError?.cause?.cause?.message ||
          '';
        const allErrorText =
          `${errorString} ${errorCause} ${nestedCause}`.toLowerCase();

        // Log para debugging
        this.logger.debug(
          `Error detection - errorString: ${errorString}, errorCause: ${errorCause}, nestedCause: ${nestedCause}`,
        );

        if (allErrorText.includes('econnrefused')) {
          if (model === 'openai') {
            errorMessage =
              'LLM Studio no está disponible. Verifica que el servidor esté ejecutándose en el puerto 1234.';
            errorCode = 'LLM_STUDIO_UNAVAILABLE';
          } else {
            errorMessage =
              'No se pudo conectar al servicio del modelo. Verifica la conexión.';
          ...
          errorMessage =
            'El modelo está tardando demasiado en responder. Intenta nuevamente o usa otro modelo.';
          errorCode = 'TIMEOUT_ERROR';
        } else if (
          allErrorText.includes('insufficient_quota') ||
          allErrorText.includes('quota')
        ) {
          errorMessage =
            'Se ha alcanzado el límite de uso del modelo. Intenta más tarde.';
          errorCode = 'QUOTA_EXCEEDED';
        } else if (
          allErrorText.includes('rate_limit') ||
      ...
      this.logger.log(
        `📤 [END] Enviando responseEnd a sala ${chatId} (${chunkCount} chunks)`,
      );
      this.logger.log(
        `✅ Respuesta completada para ${chatId} (${chunkCount} chunks)`,
      );
      if (release) release();
    } catch (error) {
  ...
  async handleJoinChat(
    ...
    const clientCount = room ? room.size : 0;
    this.logger.log(
      `👥 Cliente ${client.id} se unió al chat: ${chatId} (${clientCount} clientes en sala)`,
    );
  ...
  private processR1Content(text: string): {
    thought: string;
    response: string;
  ...
  async handleListChats(@ConnectedSocket() client: AuthSocket) {
    try {
      const sessions = await this.chatService.listChats(
        client.user?.sub ?? null,
      );
  ...
  async handleDeleteChat(
    ...
    try {
      await this.chatService.deleteChat(data.chatId, client.user?.sub ?? null);
